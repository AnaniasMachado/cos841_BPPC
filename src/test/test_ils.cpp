...
int main() {
    ...
    // -------------------- ILS parameters --------------------
    ILS::Params ils_params;
    ils_params.max_shakes = 50;
    ils_params.n_ls = 100;
    ils_params.n_sc = 25;
    ils_params.n_assign = 10;
    ils_params.shake_size = 3;
    ...
    for (int run = 1; run <= k_runs; run++) {
        ...
        BPPCSolution initial = builder.MFFD();

        ILS ils(initial, improvement,
                k1, k2, k3, ils_params,
                verbose, time_limit, max_outer_iterations);
        BPPCSolution best = ils.run();
